...
| blob storage | **cloudflare r2** (user decision 2026-07-06, anticipating high storage: 10gb free, zero egress, no 1gb cliff; supersedes the vercel-blob references in docs/plans/* — uploads use presigned puts via aws4fetch instead of @vercel/blob client uploads, serving via a custom-domain bucket). rationale for blob-vs-git unchanged ([details](research/storage-blob-vs-git.md)). client-side heic→webp downscale still mandatory. existing public/photos stays in git. strava archive on hand at ~/Desktop/export_161887324 (131 fit.gz + 41 gpx + 2 fit + activities.csv, 223 rows) |
...
| ws0 | **shipped 2026-07-06** (commits ac4f432…cdd7903): spotify crash fixed, stale-cache serving, timeouts, error boundaries, admin-gated mutations, non-destructive refresh, strava dormancy flag, blog button removed, optional blurbs, pinned posts, vitest + ci |
| ws2 | **shipped 2026-07-07**: garmin .fit upload with trim sliders + privacy button, neon + r2 pipeline, /activities/[id] detail pages with maplibre maps + svg fallback, photo carousel, calendar/currently rewired, 221 archive activities imported. intervals.icu pull = phase 2 (needs account) |
| ws3 | **shipped 2026-07-07**: git-backed cms at /admin/content (sha-guarded crud, atomic publish/unpublish moves), mdxeditor with live preview + local drafts + conflict handling, singleton editors, leetcode form with graphql autofill + auto-commit, unified multi-month admin calendar, deploy-status chip |
| ws5 | **shipped 2026-07-07**: comments (guests + google, filter chain: honeypot/blocklist/link-cap/rate-limit/dedupe), likes (signed-in, guest prompt), batched claps with daily ip cap, view counters, admin moderation at /admin/comments |
| ws6 | **shipped 2026-07-07** (visual review pending): blocks photo-essay format (backward compatible with flat sets), essay viewer with exif/location sidebar + minimap, essay builder with pre-downscale exif extraction + gps rounding, coursework detail with a-z/semester sort. ws4 (design cleanup) remains held for one-at-a-time review |
